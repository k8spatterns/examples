...
This service leverages `nodePort` to expose two ports on every cluster node. For simple testing, this approach works well. Alternatively, you can use a full ingress or load-balancer exposed route as described in the _Service Discovery_ pattern.
