...
To access the PersistentVolume used in this demo, mount a local directory into the Minikube VM with the following command:
...
You can send an HTTP request to the service to check if everything is working correctly. As we haven't installed a Service, let's use port-forwarding and `curl` to check the output:
